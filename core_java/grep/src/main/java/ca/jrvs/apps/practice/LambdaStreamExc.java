...
public interface LambdaStreamExc {
   ...
   * arbitrary number of value will be stored in an array.
   *
   * @param strings
   * @return
   */
   ...
   * e.g. filter(stringStream, "a") will return another stream which no
   * element contains "a".
   *
   * @param stringStream
   * @param pattern
  ...
  /**
   * Convert an intStream to a doubleStream and
   ...
   * Use `createIntStream` and `getLambdaPrinter` methods.
   *
   * e.g.
